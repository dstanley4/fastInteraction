...
#' @param lm_object Name of independent variable 1 column in data frame
#' @param criterion Name of independent variable 2 column in data frame
#' @param predictor1 Name of dependent variable column in data frame
#' @param predictor2 Project data frame name
#' @param center.predictors test
#' @param axis.labels test
#' @param cam.position A list with theta (degrees), phi (degrees), and distance values. Suggest default distance of 3.
#' @references
...
#'
#' # Example 1: Not mean centered
#' graph3D <- mr.plot(lm_object,
#'               criterion = exam,
#'               predictor1 = preparation,
#'               predictor2 = anxiety)
#' print(graph3D)
#'
#' # Example 2: Mean centered
#' graph3Dcentered <- mr.plot(lm_object,
#'                         criterion = exam,
#'                         predictor1 = preparation,
#'                         predictor2 = anxiety
#'                         center.predictors = TRUE)
#' print(graph3Dcentered)
#' @export
mr.plot <- function(lm_object, criterion, predictor1, predictor2, center.predictors = FALSE, axis.labels = NULL, cam.position = NULL) {
  ...
  predictor1.sub <- substitute(predictor1)
  is.predictor1 <- is.valid.name(predictor1.sub,data.col.names)
  predictor2.sub <- substitute(predictor2)
  is.predictor2 <- is.valid.name(predictor2.sub,data.col.names)
  ...
  if (all(is.predictor1==TRUE & is.predictor2==TRUE)==FALSE) {
    mesasge("Two valid predictor variables must be specified (predictor1 and predictor2).\n\n")
    return(FALSE)
  }

  predictor1.name <- deparse(predictor1.sub)
  predictor2.name <- deparse(predictor2.sub)
  criterion.name <- deparse(criterion.sub)

  predictor1.axis.name <- predictor1.name
  predictor2.axis.name <- predictor2.name
  criterion.axis.name <- criterion.name

  if (!is.null(axis.labels)){
    predictor1.axis.name <- axis.labels$predictor1
    predictor2.axis.name <- axis.labels$predictor2
    criterion.axis.name <- axis.labels$criterion
  }

  predictor1 <- data[,predictor1.name]
  predictor2 <- data[,predictor2.name]
  criterion  <- data[,criterion.name]

  surface.information <- calculate.surface(lm_object, criterion, predictor1, predictor2, criterion.name, predictor1.name, predictor2.name)
  surface.predicted.values <- surface.information$surface.predicted.values
  ...
  surface.graph <- layout(surface.graph,
                          scene = list(xaxis = list(title = predictor1.axis.name,
                                                    range = c(min(x.seq), max(x.seq)),
                                                    ticktype = "array",
                                                    tickvals = x.seq),
                                       yaxis = list(title = predictor2.axis.name,
                                                    range = c(min(m.seq), max(m.seq)),
                                                    ticktype = "array",
                                                    tickvals = m.seq),
                                       zaxis = list(title = criterion.axis.name),
